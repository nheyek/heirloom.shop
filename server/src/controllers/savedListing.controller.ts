...
export const saveListingByShortId = async (
	req: Request,
	res: Response,
) => {
	...
	const listing =
		await listingService.findFullListingDataByShortId(
			shortId,
		);
	...
	await savedListingService.saveListing(
		user.id,
		listing.id,
	);
...
export const unsaveListingByShortId = async (
	req: Request,
	res: Response,
) => {
	if (!req.userClaims?.email) {
	...
	await savedListingService.unsaveListing(
		user.id,
		listing.id,
	);
...
export const getSavedListings = async (
	...
	const listings =
		await savedListingService.getSavedListingsForUser(
			user.id,
		);
