...
export const getShop = async (
	...
	const shop =
		await shopService.findShopByShortId(shortId);
	if (!shop) {
		return res
			.status(404)
...
export const getListingsByShop = async (
	...
	const listings =
		await listingService.findListingsByShop(shop.id);
	return res.json(
		listings.map(mapListingToListingCardData),
	);
};
...
export const addListingToShop = async (
	...
	if (!req.userClaims?.email) {
		return res
			...
			.json({
				message:
					'Unauthorized: Missing email claim',
			});
	}
