...
export const deriveCombinations = (
...
): { key: string; optionIdsByVariationId: Record<string, string> }[] => {
	const sorted = Object.entries(variations).sort((a, b) => a[1].order - b[1].order);
	if (sorted.length === 0) return [];

	// Build ordered option arrays per variation
	const optionArrays = sorted.map(([varId, v]) => {
		const opts = Object.entries(v.options).sort((a, b) => a[1].order - b[1].order);
	...
	return combos.map((optionIdsByVariationId) => {
		...
		const key = sorted.map(([varId]) => optionIdsByVariationId[varId]).join('|');
