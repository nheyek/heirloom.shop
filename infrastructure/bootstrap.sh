...
"$SCRIPT_DIR/tf.sh" apply -target=${REGISTRY_RESOURCE} -auto-approve
...
"$SCRIPT_DIR/tf.sh" apply -target=${DB_CLUSTER_RESOURCE} -auto-approve

echo ">>> STEP 3: Create App Platform app (connects using seeded image)"
"$SCRIPT_DIR/tf.sh" apply -target=${APP_RESOURCE} -auto-approve

echo ">>> STEP 4: Tighten DB firewall to app-only"
"$SCRIPT_DIR/tf.sh" apply -target=${DB_FIREWALL_APP} -auto-approve
